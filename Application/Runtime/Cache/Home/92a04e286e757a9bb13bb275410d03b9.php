...
<script>
	...
	<script>
		var Sarch_text="<?php echo ($keywords); ?>";
        layui.config({
		...
		layui.use(['table','layer','jquery','laydate','form'], function(){
            ...
            Machine= table.render({
                elem:'#article',
                url: "faq.html?search_data=1&keywords="+Sarch_text //数据接口
                ,page: true //开启分页
                ,response: {
                    statusCode: 200 //规定成功的状态码，默认：0
                }
                ,cols: [[
                   {field: 'title', title: '标题',templet: '#bt',width:'20%'},
                   {field: 'content', title: '摘要',width:'80%'}
                ]]
            });

										...
										<div class="layui-form" >
											<div class="layui-form-item" style="margin-top:0">
												<ul class="nav_list">
													...
													<li class="" data-name="1">剔除被拒</li>
													<li class="" data-name="2">返无效被拒</li>
												</ul>
...
<script>
  ...
  var Machine = table.render({
    elem: '#Machine'
    ,url: 'checked.html?search_data=1' //数据接口
	//,toolbar:'#tabr'
    ,page: true //开启分页
    ,cols: [[ //表头
      {type:'checkbox'},
	  ...
	  {field: 'apply_remark', title: '申请理由'},
	  {field:'check_user',title: '审核人',},
...
form.on('submit(get_search)', function(data){
  data=data.field;
	if($('#check_time').val()){
		var Nlast = data.check_time.split('~');
		data.check_time_start=Nlast[0];
		data.check_time_end=Nlast[1];
	}
